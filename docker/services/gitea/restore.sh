...
docker compose -f "$(dirname "$0")/docker-compose.yml" stop gitea
...
docker compose -f "$(dirname "$0")/docker-compose.yml" start gitea

echo "Done. Verify the restore by checking http://<host>:3000"
